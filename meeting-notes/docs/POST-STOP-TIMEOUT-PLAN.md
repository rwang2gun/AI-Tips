...
1. **Gemini Free tier rate limit** — 30분 기다린 뒤 성공한 사실이 결정적. `gemini-2.5-flash`의 분당 RPM과 일일 토큰 한도는 좁다. 녹음 종료 순간 큐에 쌓인 세그먼트가 동시 4개(prepare 2 + transcribe 2)로 몰리고, 각각 retry까지 추가되어 quota window를 넘김. 429/`RESOURCE_EXHAUSTED` 신호.
...
- `lib/http/retry.js:18` — `RESOURCE_EXHAUSTED|UNAVAILABLE|429|503`이 retriable 분류. 즉 quota 거절 시 서버가 자동으로 시간을 더 쓴다.
