...
### Phase 1 전사 품질 증거 보존 + 스토리지 UI (2026-04-22 ~ 2026-04-23 main 머지)

- **Phase 1: 전사 품질 증거 보존 + 진단 sidecar** (커밋 `5705e02` + `1ff2575`) — 2026-04-22 AI 스터디 세션(72분) 후반부 누락 체감 분석으로 Gemini Flash generation loop가 5분 분할 내부에서도 재발함을 확인. Codex 2차 독립 리뷰를 거쳐 evidence-first 구조로 재설계. `detectLoop()`(비파괴 탐지, minRun 20회/토큰 1~20자), `transcribe-NN.meta.json`/`raw.txt` sidecar 저장, anti-loop 프롬프트 규칙(`[불분명]` 어휘 재사용), finalize cleanup 선택적 retention, manifest "전사 품질 경고" 섹션. 후속 fix로 raw.txt-first 쓰기 순서 + `retentionIndices = flagged ∪ raw.txt 존재` 합집합 + `Buffer.byteLength(raw,'utf8')` 바이트 기준 threshold(한글 1자 ≈ 3 bytes라 JS length로는 false-positive).
- **스토리지 가시화 + 수동 정리 UI** (커밋 `1c13e78`) — Phase 1의 selective retention을 대체. finalize cleanup을 **파생 파일(transcript.txt/transcript-NN.txt/result.json) 삭제 + 화이트리스트(오디오/raw/meta) 전수 보존**으로 단순화. 결과 UI에 스토리지 사용량 패널 + `[24h+ 오디오 정리]` 버튼 추가. Codex 3차 리뷰를 거쳐 `deletableBytes`(24h+ 화이트리스트) vs `stuckBytes`(failed-finalize 비-화이트리스트) 분리 — `remainingTotalBytes == prev - prev.deletableBytes` 불변식 유지. `lib/storage/usage.js`의 `isCleanupTarget`을 storage-usage/cleanup-old-audio/finalize 3곳이 공유(drift 방지). failed-finalize의 `result.json` 존재는 의도적 재시도 트리거라 cleanup에서 절대 삭제하지 않음. 계획 문서: `meeting-notes/docs/STORAGE-CLEANUP-UI-PLAN.md`.
- **Vercel Hobby 플랜 함수 한도 12개 초과 → handlers를 lib/로 이동** ([PR #17](https://github.com/rwang2gun/AI-Tips/pull/17), 커밋 `f7f1080`) — 1c13e78 배포가 production에서 `Build Failed: No more than 12 Serverless Functions` 로 실패. Vercel은 `/api/` 하위 모든 `.js` 파일을 개별 함수로 카운트하는데, `storage-usage.js`/`cleanup-old-audio.js` 2개 추가로 총 13개 → 한도 초과. `api/handlers/*.js`는 `process-meeting.js`가 `X-Action` 헤더로 디스패치하는 내부 모듈이라 독립 엔드포인트일 이유가 없어 `lib/handlers/`로 이동. `api/` 하위는 실제 엔드포인트(`process-meeting.js`, `version.js`) 2개만 남음. handler 내부 상대경로 재작성(`../../lib/*` → `../*`, legacy는 `../../../lib/*` → `../../*`). 87/87 테스트 + Vercel preview 성공 후 squash merge. **교훈**: Vercel 함수 수는 배포 단계에서만 드러나는 한도라 로컬 `npm test`/`node --check`로는 사전 감지 불가. 다음 핸들러 추가 시 `api/` 하위에 두지 말 것.

### 서버 유의어 드리프트 해소 + 발표 덱 재구성 (2026-04-20 main 머지)

- [서버 전사·요약에 유의어 사전 주입 (드리프트 해소)](work-log-archive/work_log_synonym_drift_fix.md) — 커밋 `be2fa2e`. 발표 덱 슬라이드 4 파이프라인 검증 중 **서버 전사에 유의어 미주입 + 서버 요약에도 유의어 미주입** 드리프트 발견 (PR #12 설계가 로컬 스크립트에만 구현돼 있었음). `buildSegmentTranscribePrompt`에 `synonymHint` 추가, `transcribe-segment` 핸들러에 `fetchSynonyms` + `buildTranscribeSynonymHint` + `applySynonymReplacements` regex 후처리 주입. `summarize` 핸들러에 `buildSummarizeSynonymHint` 주입. 기본값 빈 문자열로 하위호환 (`NOTION_SYNONYM_DB_ID` 미설정 시 no-op). 단위 테스트 1건 추가. 교훈 #5를 3가지 드리프트 사례(가이드·모델·유의어)로 확장.
- [발표 덱 재구성 — 2부 구조 + 섹션 커버 + 신규 사례 슬라이드](work-log-archive/work_log_deck_restructure_20260420.md) — 커밋 `3796ce6`. 14장 → 21장. 2부 구조(지금의 앱 · 만들며 겪은 일 · 정리) + Part별 색상 섹션 커버 3장. 신규 슬라이드 5건(테스트 파트너 김실장의 자동사냥, AI 리뷰 UX 구멍, 운영 3가지, 외부 유입 사건, 실전 팁 4가지). **Codex 리뷰가 `goTo()` 의 className 통째 할당이 section-cover/part-* 클래스를 매번 덮어쓰는 버그를 지적** → `classList` 기반 전환으로 수정 (디버깅 수 시간의 색상 미스터리가 단번에 해결됨).

### 크레딧 소진·모델 종료 대응 + Vertex 백엔드 (2026-06-09 ~ 2026-06-10)

2026-06-09 "회의록 앱 전송 오류" 제보로 시작된 연쇄 대응. 표면 증상은 "전송 실패 — 재시도 버튼을 눌러주세요" + 세그먼트별 원본 JSON 노출이었으나, 근본 원인은 코드 버그가 아니라 **Gemini 결제 구조**였다.

- **크레딧 소진 오류 처리 + Gemini 2.x→3.x 모델 이관** ([PR #19](https://github.com/rwang2gun/AI-Tips/pull/19), 머지 커밋 `f3d7b72`) — 제보 오류는 `429 RESOURCE_EXHAUSTED "prepayment credits are depleted"`. 그런데 `withRetry`가 429를 일괄 재시도로 분류해 서버 3회 + 클라 재시도로 ~십수 초 낭비 후 원본 JSON 노출. `lib/http/gemini-error.js` 신규 — `isBillingDepleted`(크레딧 소진=영구 실패, 재시도 제외) / `isRetriable` 분리. `process-meeting.js`가 소진 시 500 대신 **402 + 한글 안내 + `code:CREDITS_DEPLETED`** 반환 → 클라가 5xx로 오인 재시도 안 함. `app.js`의 `friendlyErrorMessage`로 세그먼트/요약 실패 모두 JSON 덩어리 숨김. 동시에 2026-06-01 Gemini 2.x 종료 대응으로 전사 `2.5-flash→3.5-flash`, 요약 `2.5-pro→3.1-pro` 이관.
- **모델 404 핫픽스 (`gemini-3.1-pro` → `gemini-3.1-pro-preview`)** (커밋 `174eaec`) — 위 이관 직후 `gemini-3.1-pro`(접미사 없음)가 **404 NOT_FOUND** 반환(정식판 미출시, preview만 존재). `gemini-error.js`에 `isModelNotFound`/`extractRequestedModel`/`describeModelNotFound` 추가 — 404는 재시도 무의미하므로 즉시 throw하되 "어떤 모델이 없는지 + 무엇을 고쳐야 하는지"를 한 줄 한글로 변환해 Vercel UI 에러 박스에 노출. `SUMMARIZE_MODEL`을 `gemini-3.1-pro-preview`로 교정.
- **`auto` 백엔드 — 선불 소진 시 Vertex 자동 폴백** ([PR #21](https://github.com/rwang2gun/AI-Tips/pull/21)) — 구독 정책 개편으로 Vertex에 적용되던 $10/월 Cloud 크레딧이 폐지(소비자 제품 사용량 한도로 대체)되어 "구독으로 API 비용 절감"은 불가해짐. 대신 **이미 충전한 AI Studio 선불을 버리지 않고 다 쓰다가, 소진되면 Vertex(후불)로 무중단 전환**하는 `auto` 모드 추가. `withBillingFallback(fn)`이 `isBillingDepleted` 감지 시 요청별로 aistudio→vertex 재시도. prepare/transcribe/summarize 각각 독립 폴백, 클라이언트 무변경(prepare가 ACTIVE 마커 반환 → 기존 폴링 단락). **모델은 `gemini-2.5-flash`/`gemini-2.5-pro`로 통일** — `gemini-3.5-flash`가 AI Studio에서 오디오 환각(실호출 확인)이라 양쪽 안전한 2.5로 회귀. Vertex 리전은 `global`(개별 리전은 최신 모델 404). 단위 테스트로 폴백 분기 검증.
- **Gemini Vertex AI 백엔드 지원** ([PR #21](https://github.com/rwang2gun/AI-Tips/pull/21)) — 사용자가 이미 Google AI Pro 구독 중이라 AI Studio 선불 크레딧과의 **이중 과금**을 피하기 위함. 핵심 발견: **AI Studio 선불 크레딧 ≠ 구독이 주는 Google Cloud 크레딧**(분리된 지갑) → 구독 크레딧을 쓰려면 Vertex AI 경유 필요. 그러나 **Vertex는 Gemini Files API 미지원** → 오디오를 인라인(base64, 100MB 한도)으로 전송하도록 재설계. `GEMINI_BACKEND` 환경변수로 듀얼 모드(기본 `aistudio`=무변경). `lib/clients/gemini-config.js`(순수 로직+단위 테스트 8건), prepare-segment는 vertex 시 `state:'ACTIVE'` 반환해 클라 폴링 자동 생략(**app.js 무변경**), transcribe-segment는 청크를 인라인 전송. GCP 미설정 상태라 실제 Vertex 호출 검증은 보류(draft). 가이드: `meeting-notes/docs/VERTEX-MIGRATION.md`.
...
6. **Gemini Flash generation loop — 입력 분할만으로 완전 방어 불가** — 5분 분할 내부에서도 재발(2026-04-22 세션 seg 01·13 재확인). 트리거는 오디오 길이보다 발화 밀도/명확도일 가능성(Phase 1 진단 sidecar로 데이터 축적 후 검증). 대응은 "치환"이 아니라 **검출 → 원본 보존 → Pro fallback** 순서. `detectLoop()`(비파괴, minRun=20), per-segment `.meta.json`/`.raw.txt` sidecar, anti-loop 프롬프트 규칙(`[불분명]` 재사용)으로 Phase 1 확정 (5705e02/1ff2575). 초기 PR #12는 길이 분할만 다뤘음.
...
10. **모델 수명주기 — 종료/preview 접미사 주의** (2026-06) — 2026-06-01 Gemini 2.0 계열 종료(권장 후속 `gemini-3.5-flash`/`gemini-3.1-flash-lite`). 정식판 미출시 모델은 `-preview` 접미사 필수 — `gemini-3.1-pro`(접미사 없음)는 404 NOT_FOUND. **404는 재시도 무의미** → 즉시 사람 친화 메시지로 변환(`describeModelNotFound`). 모델 ID는 기억(컷오프)으로 단정 말고 공식 문서/실호출로 확인
11. **두 종류의 429를 구분** (2026-06) — 일시적 한도 초과(분당 요청/모델 과부하)는 재시도 가능, **크레딧 소진**(`prepayment credits depleted`)은 영구 실패라 재시도 제외 + 결제 안내. `isBillingDepleted`로 분기. 둘 다 status는 429/RESOURCE_EXHAUSTED라 메시지로만 구분 가능
12. **AI Studio 선불 크레딧 ≠ Google Cloud/구독 크레딧** (2026-06) — 서로 다른 지갑. Google AI Pro/Ultra 구독이 주는 Cloud 크레딧은 AI Studio Gemini API(`GEMINI_API_KEY`)엔 적용 안 됨 — **Vertex AI 경유**해야 사용. 단 Vertex는 **Files API 미지원** → 오디오를 인라인(base64, 100MB 한도)으로 전송해야 함(`GEMINI_BACKEND=vertex`)
...
5. **Serverless Function 배포당 12개 한도** — `/api/` 하위 모든 `.js` 파일을 개별 함수로 카운트 (서브디렉토리 포함). 내부 디스패치 헬퍼도 `api/` 하위면 함수로 계수되어 한도 소진. 라우터 패턴(`api/process-meeting.js` + `X-Action` 헤더)을 쓸 땐 헬퍼를 `lib/handlers/`로 둬서 실제 엔드포인트만 `api/`에 남길 것. 로컬 테스트로는 사전 감지 불가 — 배포 실패 후에야 드러남 (2026-04-22 PR #17로 확인)
...
5. **서버/로컬 구현 드리프트 주의** — 반복 패턴:
   ...
   "코드 중복"이 아니라 "사용자 체감 품질이 경로에 따라 달라지는 버그". 기능 추가 시 양쪽 동시 반영 또는 공통 모듈 추출 기준 필요. **드리프트는 PR당 기본 점검 항목** — 새 Notion 참조/모델 선택/프롬프트 구성 변경 시 `lib/handlers/*`와 `scripts/*` 양쪽 모두 확인 (핸들러 위치는 PR #17에서 `api/handlers/` → `lib/handlers/`로 이동)
...
| 파일 | 역할 |
|------|------|
| `api/process-meeting.js` | Vercel 서버리스 라우터 (X-Action 헤더로 `lib/handlers/*` 디스패치 — PWA용 9단계 + legacy 2단계) |
| `api/version.js` | 배포 버전 JSON (PR #13) |
| `lib/handlers/*.js` | X-Action 핸들러 본체 — PR #17에서 `api/handlers/`에서 이동 (Vercel 12 함수 한도 회피) |
...
| `meeting-notes/docs/VERTEX-MIGRATION.md` | AI Studio→Vertex AI 백엔드 전환 가이드 (PR #21) |
...
- `GEMINI_BACKEND` (PR #21) — `aistudio`(기본) | `vertex`. vertex 시 아래 GCP 변수 필요
- `GOOGLE_CLOUD_PROJECT` / `GOOGLE_CLOUD_LOCATION` / `GOOGLE_SERVICE_ACCOUNT_JSON` (PR #21) — Vertex 모드 전용
