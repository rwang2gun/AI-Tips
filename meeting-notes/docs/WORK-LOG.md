...
### 미머지 대기 (2026-04-18 다음 세션 rebase 필요)

- **`test/summarize-pro-and-fold-ui`** — 서버 summarize Flash→Pro 전환(PR #10 서버 누락분 보완) + 완료 세그먼트 숨기기 체크박스(UI 간소화). Preview에서 기능 동작 확인. 단, 같은 브랜치에서 `3abe5062` 세션 데이터 손실 발생해 앱 버그 여부 미확정 상태라 보류.
- **`test/notion-diag-manifest`** — `진행로그_<date>_<title>.txt` Notion 첨부(서버 Blob 상태 + 세그먼트별 프리뷰 + 녹음 시각 박제). 사후 진단의 영속 경로. 위 브랜치 위에 쌓여 있음.
