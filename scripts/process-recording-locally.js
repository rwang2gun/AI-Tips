...
//
//   2) 실행:
//        node --env-file=.env scripts/process-recording-locally.js <audio-file> [--title "..."] [--type "킥오프|내부 논의|실무 논의|기타"] [--summarize-model=gemini-3.1-pro] [--transcribe-only]
//
//      --transcribe-only: 전사만 하고 요약 단계 스킵 (세그먼트별 순차 전사용)
//
//   [모델 전략]  (2026-06 Gemini 2.x 정리로 3.x 이관)
//     - 전사 (Step 1): gemini-3.5-flash (저렴 + 단순 작업에 충분)
//     - 요약 (Step 2): gemini-3.1-pro (긴 입력 + structured output에서
...
if (existsSync(resultPath) && !flags.has('--force-resummarize')) {
  ...
  // 요약은 gemini-3.1-pro 기본. Flash는 "긴 입력(100K+ chars) + structured
  ...
  const summarizeModel = kwargs['summarize-model'] || 'gemini-3.1-pro';
