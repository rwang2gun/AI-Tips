...
// 반환값: { text, flaggedSegments } — text는 Notion 첨부용, flaggedSegments는
// finalize cleanup이 해당 세그먼트 오디오/raw를 보존할 대상 판단에 사용.
// (하위 호환: 기존 호출자는 .text만 필요했지만 이제 flaggedSegments도 활용 가능.)
export async function buildManifest({
  ...
  if (flaggedSegments.length) {
    ...
    for (const s of flaggedSegments) {
      ...
      if (s.rawLength != null && s.rawLength < 2000) {
