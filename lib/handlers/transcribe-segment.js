...
export default async function handleTranscribeSegment(req, res) {
  ...
  const vertex = isVertexBackend();
  // Vertex는 fileUri 없이 청크를 인라인 전송하므로 fileMimeType만 필수.
  // aistudio는 prepare/check-file이 만든 fileUri가 반드시 있어야 함.
  if (!fileMimeType) {
    return jsonResponse(res, 400, { error: 'Missing fileMimeType' });
  }
  if (!vertex && !fileUri) {
    return jsonResponse(res, 400, { error: 'Missing fileUri' });
  }
  ...
  // 오디오 파트: aistudio는 Files API URI 참조, vertex는 청크를 다시 읽어 인라인(base64) 전송.
  // 5분 세그먼트는 ~2~5MB라 인라인 100MB 한도 안에서 안전. inlineData mimeType은 codecs 파라미터를
  // 제거한 베이스 타입(audio/webm)만 사용 — "audio/webm;codecs=opus"는 거부될 수 있음.
  let audioPart;
  if (vertex) {
