...
// gemini-2.5-flash: AI Studio·Vertex 양쪽에 제공 + 환각 없음(검증됨).
// gemini-3.5-flash는 AI Studio에서 오디오 환각(2026-06-10) → auto 폴백이 AI Studio를 거치므로 2.5로 통일.
...
export default async function handleTranscribeSegment(req, res) {
  ...
  if (!fileMimeType) {
    return jsonResponse(res, 400, { error: 'Missing fileMimeType' });
  }
  ...
  // 한 백엔드로 전사 1회 실행. 오디오 파트는 백엔드에 맞춰 구성:
  //   - aistudio: Files API URI 참조(prepare가 만든 fileUri)
  //   - vertex: 청크를 다시 읽어 인라인(base64) 전송 (Files API 미지원).
  //     5분 세그먼트는 ~2~5MB라 인라인 100MB 한도 안에서 안전. inlineData mimeType은 codecs
  //     파라미터를 제거한 베이스 타입(audio/webm)만 사용 — "audio/webm;codecs=opus"는 거부될 수 있음.
  async function runTranscribe(backend) {
    ...
    if (backend === 'vertex') {
      ...
      if (!blobs.length) {
        throw new Error(`No chunks found for segment ${segmentIndex}`);
      }
      const audioBuffer = await concatBlobChunks(blobs);
      audioPart = {
        inlineData: {
          mimeType: fileMimeType.split(';')[0].trim() || 'audio/webm',
    ...
    return withRetry(() =>
      genAI.models.generateContent({
        ...
        contents: [createUserContent([promptText, audioPart])],
  ...
  // fileUri 없음 = prepare가 Vertex로 갔다(vertex/auto-폴백) → 인라인만.
  // fileUri 있음 = aistudio 업로드 성공 → aistudio 시도, auto 모드면 소진 시 vertex 인라인으로 폴백.
  const result = fileUri
    ? await withBillingFallback((backend) => runTranscribe(backend))
    : await runTranscribe('vertex');
